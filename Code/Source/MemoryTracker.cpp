...
void Kayou::MemoryTracker::AddAllocation(const void* ptr, const std::size_t size, MemoryTag tag)
{
...
void Kayou::MemoryTracker::RemoveAllocation(const void* ptr, const std::size_t size, MemoryTag tag)
{
    const std::size_t index = static_cast<std::size_t>(tag);
...
void Kayou::MemoryTracker::ResetAll(MemoryTag tag)
{
    const std::size_t index = static_cast<std::size_t>(tag);
    s_allocated[index] = 0;
...
const char* Kayou::MemoryTracker::GetTagName(const MemoryTag tag)
{
    switch (tag)
    {
